...
from input_with_span_attr import input_with_span_attr
...
def load_ontonotes4ner(path, char_embedding_path=None, bigram_embedding_path=None, index_token=True, train_clip=False,
    ...
    train_path = os.path.join(path, 'train.char{}.bmoes'.format('.clip256' if train_clip else ''))
    dev_path = os.path.join(path, 'dev.char{}.bmoes').format('.clip256' if train_clip else '')
    test_path = os.path.join(path, 'test.char{}.bmoes').format('.clip256' if train_clip else '')
...
def load_msra_ner_without_dev(path, char_embedding_path=None, bigram_embedding_path=None, index_token=True,
                              train_clip=False,
                              char_min_freq=1, bigram_min_freq=1, only_train_min_freq=0):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
